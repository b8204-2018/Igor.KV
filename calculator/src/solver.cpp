...
std::vector<double> OrdinaryParser::parse(std::string &equation) {
    ...
    if (num.size() != 2) {
        ...
        throw std::logic_error("incorrect equation");
    }
    return num;
}
...
double Sub::calculate(std::vector<double> elements) {
    if (elements[0] == 0) {
        throw std::logic_error("incorrect input values");
    }
    return elements[0] - elements[1];
...
double Div::calculate(std::vector<double> elements) {
    if (elements[1] == 0) {
        throw std::logic_error("divider equal to zero!");
    }
    return elements[0] / elements[1];
...
std::vector<double> Quadratic::result(std::string &equation) {
    ...
    if (derivative < 0) {
        ...
        throw std::logic_error("negative derivative");
    }
    if (derivative == 0) {
...
std::vector<double> Solver::solve(short type, std::string equation) {
    ...
    if (i == n) {
        ...
        throw std::logic_error("no operation of such type");
    }
